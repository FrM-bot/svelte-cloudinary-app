<script lang="ts">
	...
	export let varint = 'default' as 'error' | 'success'
	const VARIANTS = {
		success:
			'text-green-500/80 hover:text-green-500 hover:border-green-500  border-green-500/80 hover:shadow-green-500/20',
		error:
...
>
	{#if isLoading}
		<span
			class={`${VARIANTS_LOADER[varint]} loader w-6 h-6 rounded-full border-y-transparent border-[2px] bg-transparent`}
		/>
	{:else}
		<slot />
	{/if}
